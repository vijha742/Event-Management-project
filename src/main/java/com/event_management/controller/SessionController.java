...
public class SessionController {
    ...
    @Autowired private SessionValidationService sessionValidationService;
     ...
     */
    @GetMapping("/validate")
    public ResponseEntity<SessionStatus> validateSession(Authentication authentication) {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if ("SESSION-TOKEN".equals(cookie.getName())) {
                        boolean isValid =
                                sessionValidationService.validateSessionToken(cookie.getValue());
                        return ResponseEntity.ok(new SessionStatus(isValid));
                    }
                }
            }
